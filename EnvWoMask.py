...
class SimpleThreat:
    ...
    def __init__(self, pos, speed, heading_deg, target_weapon_idx, max_turn_deg=30.0, severity=None):
        ...
        # Strategy: 1=random_wander, 2=aim_highest_asset, 3=aim_nearest_weapon
        self.strategy = 1
    ...
    def step_forward(self, dt=1.0):
        """Move forward along current heading (used for wander)."""
        heading_rad = math.radians(self.heading)
        vx = math.cos(heading_rad) * self.speed * dt
        vy = math.sin(heading_rad) * self.speed * dt
        self.pos += np.array([vx, vy])
...
# ---------------------------
# EnvWoMask (randomized asset at reset + path visualization + debug prints gated by self.debug)
# ---------------------------
class EnvWoMask(gym.Env):
    metadata = {"render.modes": ["human"]}

    def __init__(self, num_threats, num_weapons, battlefield_size, missiles_per_weapon, max_assignments, debug=True):
        """
        debug: if True prints debug info (assignments, threat list, counts...). Set to False to silence prints.
        """
        super(EnvWoMask, self).__init__()
    ...
    def _init_simple_red_team(self, num_threats, speed_range=(1.0, 4.0), max_turn_deg=30.0, seed=None):
        ...
        for i in range(num_threats):
            ...
            # assign a random strategy per threat (1,2,3)
            thr.strategy = int(np.random.choice([1, 2, 3]))
        ...
        if self.debug:
            ...
            strategy_map = {1: "random_wander", 2: "aim_highest_asset", 3: "aim_nearest_weapon"}
            strat_list = [f"T{idx}:{strategy_map.get(getattr(t, 'strategy', 1))}" for idx, t in enumerate(self.simple_threat_objs)]
            print(f"Threat strategies (threat_idx:strategy): {strat_list}")

    def _step_simple_threats_motion(self, dt=1.0, reach_threshold=2.0):
        """
        Step threats according to their assigned strategy:
           1 -> random_wander: small random turn then step_forward()
           2 -> aim_highest_asset: compute currently highest-asset active weapon and step_towards it
           3 -> aim_nearest_weapon: find nearest active weapon and step_towards it
        """
        for thr in list(self.simple_threat_objs):
            strat = getattr(thr, "strategy", 1)
            if strat == 1:
                # random wander: small random heading change then move forward
                thr.heading = (thr.heading + float(np.random.uniform(-thr.max_turn_deg, thr.max_turn_deg)) * dt) % 360.0
                thr.step_forward(dt=dt)
            elif strat == 2:
                # aim highest asset (if there are weapons)
    ...
    def step(self, action):
        # Print weapon asset values each step for visibility (if debug)
        if self.debug:
            print(f"\n🎯 Weapon asset values (episode): {self.weapon_asset_values.tolist()}")

        # ===  print compact threat evaluation list (if debug) ===
        if len(self.threats) > 0 and self.debug:
        ...
        #     reshaped = np.array(action).reshape(self.num_weapons, self.missiles_per_weapon)
        #     accepted = []
        #     for w in range(self.num_weapons):
        #         row = []
        #         for m in range(self.missiles_per_weapon):
        #             sel = int(reshaped[w,m])
        #             if 0 <= sel < len(self.threats):
        #                 row.append(sel)
        #             else:
        #                 row.append(None)
        #         accepted.append(row)
            
        #     print("🟡 Accepted selections (None = invalid):", accepted)
        self.steps += 1
        reward = 0
        done = False

        # Move threats (now strategy-aware)
        ...
        # Forced assignment if some threats left unassigned and missiles available (print those too if debug)
        ...
        # After processing, print compact assignment summary (if debug)
        if self.debug:
    ...
    def render(self, action=None):
        ...
        strategy_map = {1: "wander", 2: "aim_high", 3: "aim_near"}
        for i, t in enumerate(self.simple_threat_objs):
            ...
            strat_label = strategy_map.get(getattr(t, "strategy", 1), "unk")
            plt.text(t.pos[0] + 1, t.pos[1] + 1, f"T{i}->W{t.target_weapon_idx} S:{strat_label}", fontsize=9, color='black')
