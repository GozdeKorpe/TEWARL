...
class TEWAEnv(gym.Env):
    ...
    def __init__(self, num_threats=4, num_weapons=3, battlefield_size=100, missiles_per_weapon=10, max_assignments=1):
        ...
        # Observation space: [Threat positions, speeds, orientations, severity, missile availability]
        self.observation_space = spaces.Box(
            low=0, high=battlefield_size, shape=(num_threats * 5 + num_weapons*3,), dtype=np.float32
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        padded_threats = np.zeros((self.max_threats, 5))  # Create a zero matrix
    ...
    def evaluate_threats(self):
        ...
        for i in range(self.num_threats):
            ...
            danger_score = (
                ...
                (speed_score * 0.1)       # Faster threats are slightly more dangerous
            )

            # Ensure the final score is between 0-100
            danger_score = np.clip(danger_score, 0, 100)

            threat_evaluation.append((i, danger_score, distance_score, heading_score, severity_score, speed_score))
    ...
    def step(self, action):
        ...
        for weapon_idx in range(self.num_weapons):
            for missile_idx in range(self.missiles_per_weapon):
                if self.weapons[weapon_idx, 2] > 0:  # Check if weapon has missiles left
        ...
        for threat_idx in range(self.num_threats):
            if threat_idx not in assigned_this_step and threat_idx in threat_ranking:
                low_priority_penalty = (threat_ranking[threat_idx] / len(threat_evaluation)) * -1  
        ...
        padded_threats = np.zeros((self.max_threats, 5))  # Create a zero matrix of max size
